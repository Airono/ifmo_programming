...
class Student {
    ...
    public:
        ...
        void input(int scores[5], int i, int sum) {
            cout << "Score " << i + 1 << ": ";
            cin >> scores[i];
            if(scores[i] > 5)
                    throw ExScore("In function input()", scores[i]);
...
int main() {
    ...
    for (int i = 0; i < 5; ++i)
        try {
            student01.input(scores, i, sum);
        } catch(Student::ExScore ex) {
            cout << "\nInput error " << ex.origin;
            cout << "\nInput mark " << ex.iValue << "is invalid\n";
        }
    ...
    try {
        ...
        cout << "\nInput marks " << ex.iValue << "is invalid\n";
    }
